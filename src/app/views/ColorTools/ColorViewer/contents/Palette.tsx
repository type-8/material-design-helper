...
import CopiedSnackbar from '../../snackbar/Copied';
import type { OpenCopiedSnackbarRef } from '../../snackbar/Copied';
import { useColorViewer } from '../Context';
import type { ColorViewerKeys } from '../Context';
      ...
      <div class={styles.content}>
        <header class={styles.header}>
          <span class={styles['header-text']}>
            { key }
            <Show when={color()}>{(value) => <span class={styles['header-subtext']}> - { value.label } - { value.shade }</span>}</Show>
          </span>
        ...
        <Show when={color()} fallback={createMonoColorElement()}>{({ order, value }) => {
          ...
          return (
            <div class={styles.color}>
              <div class={`${styles['top-color']} cvt-${first}-${key}`} onClick={(event) => openCopiedSnackbar.ref(value[first].hex, event)}>
                <span>{ first }</span>
                <span class={styles['color-code']}>{ value[first].hex }</span>
              </div>
              <div class={`${styles['bottom-color']} cvt-${second}-${key}`} onClick={(event) => openCopiedSnackbar.ref(value[second].hex, event)}>
                <span>{ second }</span>
                <span class={styles['color-code']}>{ value[second].hex }</span>
              </div>
              <div class={`${styles['bottom-color']} cvt-${third}-${key}`} onClick={(event) => openCopiedSnackbar.ref(value[third].hex, event)}>
                <span>{ third }</span>
                <span class={styles['color-code']}>{ value[third].hex }</span>
              </div>
            </div>
          );
        }}</Show>
